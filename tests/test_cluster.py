...
class TestCluster(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        links1 = [[points1[l] for l in [random.randint(0, lim1-1) for x in xrange(linknum)]] for i in xrange(lim1)]
        points2 = [np.array([float(i)/lim1, float(j)/lim2]) for i in xrange(lim1) for j in xrange(lim2)]
        links2 = [[points2[l] for l in [random.randint(0, (lim1-1)*(lim2-2)) for x in xrange(linknum)]]
        ...
        links3 = [[points3[l] for l in [random.randint(0, (lim1-1)*(lim2-1)*(lim3-1)) for x in xrange(linknum)]]
        ...
        dist_links1 = [[dist_points1[l] for l in [random.randint(0, lim1 - 1) for x in xrange(linknum)]]
                       for i in xrange(lim1)]
        dist_points2 = [np.array([2+float(i) / lim1, 2+float(j) / lim2]) for i in xrange(lim1) for j in xrange(lim2)]
        dist_links2 = [[dist_points2[l] for l in [random.randint(0, (lim1 - 1) * (lim2 - 2)) for x in xrange(linknum)]]
                  for j in xrange(lim2) for i in xrange(lim1)]
        ...
        cls.dist_check3 = np.linalg.norm(np.array([2-float(lim1 - 1) / lim1, 2-float(lim2 - 1) / lim2,
                                              2-float(lim3 - 1) / lim3]))
